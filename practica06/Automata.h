...
class Automata {
 public:
  ...
  // Método que dice si es un DFA o no. --> modificacion
  bool EsDfa();
  // Método epsilon clausura
  std::map<int, Estado> EpsilonClausura(std::map<int, Estado> estados);
  // Método mover
  std::map<int, Estado> Mover(std::map<int, Estado> estados, Simbolo simbolo);
  // Algoritmo de construccion de subconjuntos
  Automata ConstruccionSubconjuntos();
 private: 
  std::map<int, Estado> estados_;  // identificador -> nombre estado
