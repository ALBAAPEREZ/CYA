...
// Contiene la clase Estudiantes en la que tenemos como atributi privado el contenedor multimap
...
// se le pasa el fichero para abrirlo y el propio getter del multimap.
...
class Estudiantes {
 public:
  ...
  std::multimap<std::string, double> GetEstudiantes() const { return estudiantes_;}
  void MostrarEstudiantes();
  void InsertarEstudiante(const std::string& alu_estudiante, double nota_estudiante);

 private: 
  std::multimap<std::string, double> estudiantes_;
};
